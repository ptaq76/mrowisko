...
@section('title', 'Archiwum – Załadunki')
...
.filter-group input:focus { border-color:#7f8c8d; }

.btn-copy-date { display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:7px;cursor:pointer;background:#f4f5f7;color:#888;font-size:14px;border:1px solid #dde0e5;transition:all .15s ease; }
.btn-copy-date:hover { background:#7f8c8d;color:#fff;border-color:#7f8c8d; }

.btn-filter { padding:8px 18px;background:#7f8c8d;color:#fff;border:none;border-radius:7px;font-size:13px;font-weight:700;cursor:pointer; }
.btn-filter:hover { background:#626c6d; }

.btn-back { padding:8px 16px;background:#f4f5f7;color:#555;border:1px solid #dde0e5;border-radius:7px;font-size:13px;font-weight:700;cursor:pointer;text-decoration:none;display:flex;align-items:center;gap:6px; }
...
<div class="report-wrap">
    ...
    <div class="report-header">
        <div class="report-title">
            <i class="fas fa-archive" style="color:#7f8c8d"></i>
            Archiwum Załadunków
            <span class="badge-count">{{ $orders->count() }}</span>
        </div>
        <a href="{{ route('biuro.reports.loadings') }}" class="btn-back">
            <i class="fas fa-arrow-left"></i> Powrót do raportu
        </a>
    </div>

    <form method="GET" action="{{ route('biuro.reports.loadings.archived') }}">
        <div class="filters">
            ...
            <a href="{{ route('biuro.reports.loadings.archived') }}" style="font-size:12px;color:#aaa;align-self:center">
...
async function unarchive(id) {
    const conf = await Swal.fire({
        title: 'Przywrócić załadunek?',
        icon: 'question', showCancelButton: true,
        confirmButtonColor: '#f39c12',
        confirmButtonText: 'Tak, przywróć', cancelButtonText: 'Anuluj',
    });
    if (!conf.isConfirmed) return;
    const res  = await fetch(`/biuro/reports/loadings/${id}/unarchive`, {
        method: 'POST',
