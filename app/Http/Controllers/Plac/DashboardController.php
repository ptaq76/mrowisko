...
class DashboardController extends Controller
{
    ...
    public function loadingForm(Order $order)
    {
        ...
        $date = $order->planned_date ?? \Carbon\Carbon::today();

        $fractions = \App\Models\WasteFraction::forLoadings()->orderBy('name')->get();
    ...
    public function loadingAdd(Order $order)
    {
        ...
        foreach ($fractions as $f) {
            $s = \App\Models\WarehouseItem::selectRaw('SUM(bales) as b, SUM(weight_kg) as w')
    ...
    public function loadingEdit(Order $order, \App\Models\LoadingItem $item)
    {
        $order->load(['client', 'driver']);
